...
public class Application implements CommandLineRunner {
	...
	public void run(String... strings) throws Exception {
		...
		// Flight[] flights =
		// searchClient.postForObject("http://search-service/search/get", searchQuery,
		// Flight[].class);
		...
		try {
			// long bookingId =
			// bookingClient.postForObject("http://book-service/booking/create", booking,
			// long.class);
		...
		if (bookingId == 0)
			return;
		try {
			// CheckInRecord checkIn = new CheckInRecord("Franc", "Gavin", "28C", null,
			// "BF101","22-JAN-16", bookingId);
			CheckInRecord checkIn = new CheckInRecord("Franc", "Gavin", "28C", new Date(), "BF101", "22-JAN-16",
					bookingId);
